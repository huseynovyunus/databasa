...
// ----------------------------------------------------
// ⚙️ AZURE PUPPETEER KONFİQURASİYASI
// ----------------------------------------------------
...
// 2. YouTube Məlumat Çıxarma (Yenilənmiş versiya)
...
        };
    } catch (error) {
        // OEmbed uğursuz olarsa, yalnız thumbnail-i qaytarırıq.
        // Başlıq və Təsvir üçün Puppeteer-ə etibar edəcəyik (Əsas Məntiqdə).
        return {
            thumbnail: `https://img.youtube.com/vi/${videoId}/maxresdefault.jpg`,
            title: null, // Boş qoyuruq ki, aşağıda Puppeteer-in tapdığı istifadə olunsun
            description: null, // Boş qoyuruq ki, aşağıda Puppeteer-in tapdığı istifadə olunsun
...
            executablePath: AZURE_EXECUTABLE_PATH,
            headless: 'new',
...
            // YouTube-da OEmbed uğursuz olsa belə, thumbnail mövcud olur
...
        // 2. Puppeteer ilə dərin çıxarma (YouTube OEmbed uğursuz olarsa VƏ ya plan pulsuz olmasa)
        // Qeyd: YouTube üçün, OEmbed uğursuz olsa belə, dəqiq başlıq və təsvir üçün Puppeteer-i işə salmalıyıq.
        if (extractionPlan !== PRICING_PLANS.FREE.internal || isYouTubeUrl) {
            context.log(`[API]: ${extractionPlan.toUpperCase()} planı/YouTube üçün dərin çıxarma işə salınır...`);
            const deepResult = await extractDeepData(url, extractionPlan, context);
            
            // Puppeteer nəticələri ilə OEmbed nəticələrini birləşdirmək
            if (!data.title || data.title === 'Başlıq tapılmadı') data.title = deepResult.title;
            if (!data.description || data.description === 'Təsvir tapılmadı') data.description = deepResult.description;
